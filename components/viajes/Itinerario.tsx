...
function clashingTimes(items: ItineraryItem[]): Set<string> {
  const counts: Record<string, number> = {};
  items.forEach(it => { if (it.time.trim()) counts[it.time] = (counts[it.time] || 0) + 1; });
      ...
      {days.map(d => (
        <Card key={d.id}>
          ...
          <div className="flex flex-col gap-3" style={{ position: "relative" }}>
            ...
            {(() => {
              const clashes = clashingTimes(d.items);
              return d.items.map(it => {
                const clashing = it.time.trim() && clashes.has(it.time);
                return (
                  <div key={it.id}>
                    <div className="flex items-center gap-2" style={{ position: "relative" }}>
                      <span style={{ position: "absolute", left: 34, width: 5, height: 5, borderRadius: 999, background: clashing ? C.coral : C.teal, zIndex: 1 }} />
                      <input value={it.time} onChange={e => updateItem(d.id, it.id, { time: e.target.value })}
                        ...
                        style={{ ...inputStyle, width: 72, fontFamily: F.mono, fontSize: 12, border: clashing ? `1px solid ${C.coral}` : inputStyle.border }} />
                    ...
                    {clashing && (
                      <div className="flex items-center gap-1" style={{ marginLeft: 84, marginTop: 2 }}>
                        <AlertTriangle size={11} color={C.coral} />
                        <span style={{ fontFamily: F.mono, fontSize: 10, color: C.coral }}>Coincide en horario con otra parada de este día</span>
                      </div>
                    )}
                  </div>
                );
